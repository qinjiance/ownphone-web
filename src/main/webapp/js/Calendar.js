...
strFrame+='        <td colspan=5 align=right style="color:#1478eb"><span onclick="" style="font-size:12px;cursor: hand"';
strFrame+='         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title=""></span>&nbsp;&nbsp;<span onclick="parent.meizzToday()" style="font-size:12px;cursor: hand"';
...
strFrame+='          <tr bgcolor="#63A3E9"><td id=bUseTimeLayer width=30  style="cursor:hand" title=""';
...
function setday(useTime,tt,obj) //主调函数
{
	//alert(1);
	if (arguments.length > 3){alert("对不起！传入本控件的参数太多！");return;}
	if (arguments.length <=1 ){alert("对不起！您没有传回本控件任何参数！");return;}
	var dads = document.all.endDateLayer.style;
	var th = tt;
	var ttop = (arguments.length == 2) ? tt.offsetTop : obj.offsetTop;	//TT控件的定位点高
	var thei = (arguments.length == 2) ? tt.clientHeight : obj.clientHeight;	//TT控件本身的高
	var tleft = (arguments.length == 2) ? tt.offsetLeft : obj.offsetLeft;	//TT控件的定位点宽
...
function SetTimeNow(obj)
{
	...
	var cur_second	= new Date().getSeconds();
	...
	if(obj && obj.length>0)
	{
		...
		if(o)
		{
		o.value= cur_year + "-" + format(cur_month) + "-" + 
		format(cur_date) + " " + format(cur_hour) + ":" +
		format(cur_minute) + ":" + format(cur_second); 
		}
